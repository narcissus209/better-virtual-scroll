...
<script setup lang="ts" generic="T extends { id: string | number; height?: number }">
...
watch(
  ...
  async () => {
    ...
    await nextTick()
    memberTop = Math.max(props.scrollTop, 0)
    scrollTo({ top: props.scrollTop, behavior: 'instant' })
  },
  {
    immediate: true,
  },
